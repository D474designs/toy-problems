...
var powerSet = function(str) {
 ...
 var hash = {};
 if (!str) { str = ''; }
 str = str.split('').sort();  // remove duplicates
 ...
 var recurse = function(strSet) {
   var joined = strSet.join('');    // check if we have visited this combo
   if (hash[joined]) { return; }
   hash[joined] = true;
   set.push(joined);    // don't recurse to empty set - add it once at the end
   if (strSet.length === 1) { return; }    // recurse all subsets
 ...
 recurse(str);
 set.push(''); // the power set, by definition, includes the empty set  return set;
 /* END SOLUTION */
};
